# DSH Credential Center

一个带 Web 页面、面向其他 DSH Host 插件提供读取服务的密钥变量管理插件。它的使用方式类似环境变量：用户在“凭据中心”页面新增 `DEEPSEEK_API_KEY` 等变量，其他插件通过 `ctx.credentialVariables` 按变量名读取当前值。
...
```powershell
dsh plugin --profile my-profile add .\dist\dsh-credential-center-0.1.0.tgz --ignore-scripts
```

重新启动该 profile 后，左侧栏会出现“凭据中心”。
