...
export class VariableStoreError extends Error {
  constructor(readonly code: 'VALIDATION' | 'NOT_FOUND' | 'CONFLICT', message: string) {
    super(message)
...
function validateDocument(raw: unknown): VariableDocument {
  ...
  if (!Number.isInteger(document.revision) || (document.revision as number) < 0) throw new Error('revision is invalid')
  ...
  for (const [name, unknownEntry] of Object.entries(document.variables as Record<string, unknown>)) {
    if (!isVariableName(name) || typeof unknownEntry !== 'object' || unknownEntry === null) throw new Error(`variable ${name} is invalid`)
    const entry = unknownEntry as Record<string, unknown>
    if (typeof entry.description !== 'string' || typeof entry.value !== 'string' || entry.value.length === 0 || typeof entry.updatedAt !== 'string') {
...
export class VariableStore {
  ...
  async remove(name: string): Promise<VariableSnapshot> {
    return this.mutate(async () => {
      if (this.document.variables[name] === undefined) throw new VariableStoreError('NOT_FOUND', `variable "${name}" does not exist`)
...
export function defaultDataDir(): string {
  ...
  const base = resolve(override !== undefined && override.length > 0 ? override : join(homedir(), '.dsh'))
  return join(base, 'dsh-credential-center')
}
