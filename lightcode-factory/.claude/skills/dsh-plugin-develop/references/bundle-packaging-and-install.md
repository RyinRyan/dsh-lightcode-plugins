...
## npm cache 与离线 staging

`--offline` 只禁止网络，不会自动把普通依赖放进新 cache。若 staging 中的成员还依赖 `zod` 等 registry 包，cache 必须已经包含其元数据和 tarball。

- 从 `npm ci/install` 阶段开始就使用同一个可写 cache，打包时继续复用它；不要在 pack 前临时切换到空目录。
- Windows sandbox 无权写用户 npm cache 时，可在 workspace 的忽略目录使用并预热本地 cache；如果现有 lockfile 依赖无法离线预热，则经用户授权在 sandbox 外运行 pack。
- 记录 pack 使用的 cache 策略；`EPERM` 或 `ENOTCACHED` 时不得跳过 tarball 验证。
- 最终 tarball 仍要检查不存在绝对 `file:` 依赖，cache 只用于构建，不能进入产物。
