...
- 单元/组合测试证明业务契约，不证明 tarball 或浏览器装配。
- tarball + dump-config 证明可解析装配，不证明核心交互。
- Web 根路径 200 只证明 Host 启动，不证明页面、Remote 或 workflow 可用。
...
4. 从目标版本的 Browser module inventory、Network 或宿主日志确认实际 Client 资源 URL；只有版本契约明确时才按固定 `/plugins/<id>/client.js` 路径检查，不能用猜测的 404 判定插件失败。
