...
export function InstallerPanel({ api, t }: { readonly api: InstallerApi; readonly t: Translate }) {
  ...
  const remove = async (name: string): Promise<void> => {
    if (!window.confirm(t('remove.confirm', { name }))) return
    setBusy(true); setError(null)
    try { await api.remove(name); await refresh() }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) }
    finally { setBusy(false) }
  }
  ...
  return <main className="dti-root"><div className="dti-shell">
    ...
    <section className="dti-card">
      ...
      {running && <div className="dti-alert"><span className="dti-spinner"/>{t(operation.kind === 'remove' ? 'removing' : 'installing')} {operation.packageName}@{operation.version}</div>}
      {operation?.state === 'succeeded' && preview === null && <><div className="dti-alert"><strong>{t(operation.kind === 'remove' ? 'remove.success' : 'success')}</strong><br/>{operation.activation?.state === 'live' ? t(operation.kind === 'remove' ? 'remove.live' : 'live') : <>{t('restart')}{operation.activation?.reason && `：${operation.activation.reason}`}</>}</div><div className="dti-actions"><button className="dti-button secondary" onClick={() => setSnapshot({ ...snapshot!, operation: null })}>{t('retry')}</button></div></>}
    ...
    <section className="dti-card dti-installed">
      ...
      {snapshot === null ? <div className="dti-muted dti-list-state"><span className="dti-spinner"/>{t('installed.loading')}</div> : packages.length === 0 ? <div className="dti-muted dti-list-state">{query ? t('installed.noMatch') : t('installed.empty')}</div> : <div className="dti-package-list">
        {packages.map(item => <article className="dti-package" key={item.name}><div className="dti-package-main"><strong>{item.name}</strong>{item.description && <span>{item.description}</span>}</div><div className="dti-package-meta"><span>{packageVersion(item)}</span><span>{t(`source.${item.source}`)}</span><span className={`dti-package-kind ${item.isDshPlugin ? '' : 'plain'}`}>{t(item.isDshPlugin ? 'installed.plugin' : 'installed.package')}</span>{item.name !== 'dsh-tar-installer' && <button className="dti-text-button" disabled={busy || running} onClick={() => void remove(item.name)}>{t('remove.action')}</button>}</div></article>)}
      </div>}
    </section>
  </div></main>
}
